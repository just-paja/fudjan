...
namespace System
{
  ...
  class User extends Model\Perm
  {
    ...
    public static function startSession(\System\Http\Request $rq, $login, $password)
    {
      $user = static::get_first(array("login" => $login))->fetch();

      if (!$user) {
        throw new \System\Error\NoSuchObject('cannot-login');
      }

      if ($user->login($rq, $password)) {
        return true;
      }
