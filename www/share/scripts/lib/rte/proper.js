...
(function(){
	...
	<div class="proper-commands"> \
		...
		<a href="#" title="Close" class="command close" command="close"><div>Close</div></a>\
		<br class="clear"/>\
	</div>';
	...
	function activate (el, opts) {
		...
		$('html').bind('click.rte', function() {
			pwf.rte.deactivate();
		});

		// Make editable
		$(el).attr('contenteditable', true).addClass('active');
		activeElement = el;
		bindEvents(el);
