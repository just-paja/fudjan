...
namespace System\Model
{
	abstract class Ext extends Basic
	{
		...
		static function get_all($model, array $conds = array(), array $opts = array(), array $joins = array())
		{
			...
			if (!in_array("Core\System\ExtModel", class_parents($model)) || !any($attrs = self::get_model_attrs($model))) {
		...
		static function find($model, $ids = NULL, $force_array = false)
		{
			if (is_array($ids) || ($ex = strpos($ids, ','))) {
				...
				$ex && $ids = explode(',', $ids);
				$conds = array(BasicModel::get_id_col($model). " IN ('" .implode('\',\'', $ids)."')");
				return self::get_all($model, $conds)->fetch();

			} else {

				$col = BasicModel::get_id_col($model);
				if (!is_numeric($ids)) {
		...
		public static function get_model_attrs($model)
		{
			...
			if (!is_array(Cache::fetch('extmodel-attrs-'.$model, $attrs))) {
				if ($set = Cache::get('extmodel-attr-set-'.$model)) {
					...
					$helper = BasicModel::get_all("\Core\Attr\Set", array("`object_class` = '".self::clear_model_name($model)."'"), array());
					$helper->join('attr', "USING(id_attr_set)", 'ta');
					$helper->add_cols(array_merge((array) BasicModel::get_id_col("\Core\Attr"), BasicModel::get_model_attrs("\Core\Attr")), 'ta');
		...
		public static function get_model_attr_groups($model)
		{
			...
			if (!Cache::fetch('groups-'.$model, $groups) && self::get_model_attr_set($model)) {
				$groups = Cache::set('groups-'.$model, BasicModel::get_all("\Core\Attr\Group", array("id_attr_set" => self::get_model_attr_set($model)->id))->fetch());
			} else {
				$groups = array();
			}
		...
		public function update_attrs(array $update)
		{
			foreach ($attrs = self::get_model_attrs($this) as $attr)
			{
				...
				BasicModel::fix_datatypes(array($attr->seoname), $attr->type, $this->ext);
